...
pkgs <- c("tarchetypes", "crew", "reproj", "sds", "jsonlite", "vapour", "targets", "dplyr", "glue", "digest", "gdalraster", "minioclient")
...
if (Sys.getenv("AWS_ACCESS_KEY_ID") == "") { 
            Sys.setenv( AWS_ACCESS_KEY_ID = Sys.getenv("PAWSEY_AWS_ACCESS_KEY_ID"), AWS_SECRET_ACCESS_KEY = Sys.getenv("PAWSEY_AWS_SECRET_ACCESS_KEY") )
}
tar_option_set( controller = if (ncpus <= 1) NULL else crew_controller_local( workers = ncpus, options_local = crew_options_local(log_directory = log_directory) ), format = "qs", packages = pkgs )
...
collection <- c("sentinel-2-c1-l2a") |> tar_target()
  ...
  # Branch here for parallel STAC fetching Need to group querytable first
  querytable_grouped <- querytable |> dplyr::mutate(row_id = dplyr::row_number()) |> dplyr::group_by(row_id) |> tar_group() |> tar_target(iteration = "group")
  
  # Execute STAC queries (one per row)
  stac_json_list <- getstac_json(querytable_grouped) |> tar_target(pattern = map(querytable_grouped), iteration = "list")
  
  # Join results back (by key, not position)
  stac_json_table <- join_stac_results(querytable, stac_json_list) |> tar_target()
  ...
  # Filter to only NEW solardays (> last processed) This is critical for incremental processing
  ...
  images_table <- tidyr::unnest(stac_tables_new |> dplyr::select(-js), cols = c(assets)) |> dplyr::group_by(location, solarday, provider) |> tar_group() |> tar_target(iteration = "group")
  
  # === IMAGE PROCESSING === Build Scene Classification Layer (SCL) for cloud detection
  scl_tifs <- build_scl_dsn(images_table, root = rootdir) |> tar_target(pattern = map(images_table))
