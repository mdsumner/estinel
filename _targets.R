...
tar_assign(
  ...
  crslaea <- mk_crs(lon, lat) |> tar_target( pattern = map(lon, lat), iteration = "list")
  ## note that mkextent and unproj are vectorized over matrix rows
  extentlaea <- mkextent(cbind(lon, lat), radiusy, radiusx, cosine = FALSE) |> tar_target(pattern = map(lon, lat), iteration = "list")
 ...
 extent <- vaster::buffer_extent(reproj::reproj_extent(extentlaea, crs, source = crslaea), resolution) |> 
   tar_target(pattern = map(extentlaea, crs, crslaea), iteration = "list")
